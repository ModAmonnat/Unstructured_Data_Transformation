...
- **Phase 1**: Identifying the elements and patterns we need for final output. Then test if the script detects the right elements by printing the result so we can manually check the results against the original file.
- **Phase 2**: Identify the header boundary, extract transactions using regex, and return structured data as a list of dictionaries.
